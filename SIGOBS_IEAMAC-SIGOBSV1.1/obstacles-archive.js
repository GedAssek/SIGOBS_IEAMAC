...
// Fonction de filtrage pour restreindre les archives à l'aérodrome courant
function eventBelongsToCurrentAerodrome(ev) {
  const currentId = App.aerodromeMongoId;
  if (!currentId) return true;

  if (ev.collection === 'Aerodromes' || ev.collection_impactee === 'Aerodromes') {
    if (ev.document_id === currentId) return true;
    if (ev.document_id && ev.document_id._id === currentId) return true;
    if (ev.id === currentId) return true;
  }

  if (ev.aerodrome_id === currentId) return true;
  if (ev.aerodrome_id && ev.aerodrome_id._id === currentId) return true;

  if (ev.document_id && ev.document_id.aerodrome_id) {
    const docAeroId = ev.document_id.aerodrome_id._id || ev.document_id.aerodrome_id;
    if (docAeroId === currentId) return true;
  }

  if (ev.nouvelles_valeurs && ev.nouvelles_valeurs.aerodrome_id) {
    const nAeroId = ev.nouvelles_valeurs.aerodrome_id._id || ev.nouvelles_valeurs.aerodrome_id;
    if (nAeroId === currentId) return true;
  }
  if (ev.anciennes_valeurs && ev.anciennes_valeurs.aerodrome_id) {
    const oAeroId = ev.anciennes_valeurs.aerodrome_id._id || ev.anciennes_valeurs.aerodrome_id;
    if (oAeroId === currentId) return true;
  }
...
async function fetchArchiveEntries(forceRefresh = false) {
  ...
  try {
    ...
    let events = res.data || (Array.isArray(res) ? res : []);
    
    events = events.filter(eventBelongsToCurrentAerodrome);
