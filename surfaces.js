    ...
    if (map.getSource('src-backend-surfaces')) {
      ...
      map.on('click', 'backend-surfaces-extrude', (e) => {
        ...
        const html = `<div style="font-family:monospace;font-size:11px;color:#d0e8f5;
          ...
          ${p.altitude_sommet != null ? `<span style="color:#6A8AA8">Sommet :</span> <b>${Number(p.altitude_sommet).toFixed(0)} ft</b><br>` : ''}
          ${p.altitude_base != null ? `<span style="color:#6A8AA8">Base :</span>   <b>${Number(p.altitude_base).toFixed(0)} ft</b><br>` : ''}
        </div>`;
