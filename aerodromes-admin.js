  ...
  container.innerHTML = App.aerodromesList.map(a => {
    ...
    <div class="runway-mgmt-item">
      ...
      <div class="runway-mgmt-actions">
        ...
        <button class="action-btn" onclick="openEditAerodromeModal('${id}', '${(a.code_oaci || a.icao || '').replace(/'/g, "\\'")}', ${a.altitude || 0}, ${a.var || 0})">ÉDITER</button>
...
function openEditAerodromeModal(id, icao, currentAlt, currentVar) {
  showModal(
    `Éditer l'aérodrome ${icao}`,
    `<div class="field-group" style="margin-bottom:10px;">
       <label class="field-label">ALTITUDE (m)</label>
       <input type="number" id="edit-aero-alt" class="field-input" value="${currentAlt}" step="0.1" />
     </div>
     <div class="field-group">
       <label class="field-label">VARIATION MAGNÉTIQUE (°)</label>
       <input type="number" id="edit-aero-var" class="field-input" value="${currentVar}" step="0.1" />
     </div>`,
    async () => {
      const alt = parseFloat(document.getElementById('edit-aero-alt')?.value);
      const magVar = parseFloat(document.getElementById('edit-aero-var')?.value);
      if (isNaN(alt) || isNaN(magVar)) {
        showToast('Altitude et variation sont requises', 'warn');
        return;
      }
      try {
        await apiFetch(`/aerodromes/${id}`, 'PATCH', { altitude: alt, var: magVar });
